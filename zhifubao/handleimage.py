...
def match_img_single(whole_img, click_img, ignore_img_pos_list, w, h, is_ignore):
    ...
    try:
        print("匹配" + cur_img_type + ", 开始 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>is_ignore=%d" % is_ignore)
        ...
        data2 = []
        # 总是把首个坐标加入到 data2 中
        # if is_ignore is False or is_ignore is True:
        if data:
            data2.append([data[0][0] + w // 2, data[0][1] + h // 2])
            print("    首个坐标加入 ____data[0][0]=%d, data[0][1]=%d, newX=%d, newY=%d"
                  % (data[0][0], data[0][1], data[0][0] + w // 2, data[0][1] + h // 2))
        ...
        for i in range(1, len(data)):
            ...
            if data[i][0] > data[i - 1][0] + 10 or data[i][0] < data[i - 1][0] - 10:
                if data[i][1] > data[i - 1][1] + 10 or data[i][1] < data[i - 1][1] - 10:
                    ...
                    print("    data2--1--不加入坐标--坐标-- data[i][0]=[%d], data[i - 1][0] + 10=[%d]" % (data[i][0], data[i - 1][0] + 10))
                    pass
                ...
                print("    data2--3--不加入坐标--入坐标-- [%d, %d]" % (data[i][0] + w // 2, data[i][1] + h // 2))
...
def is_ignore_images(cur_img_pos_y, ignore_img_pos_list):
    if ignore_img_pos_list is None:
        return False
...
def match_img(click_img_list, ignore_img_list):
    ...
    for i in range(0, len(click_img_list)):
        ...
        idx = idx + 1
        print('检测到到第%d个点击图片按钮, %s' % (idx, img_element.imgPath))
        img_pos_list = match_img_single(screen_cap_img,
                                        img_element.imgObj,
                                        ignore_img_pos_list,
                                        img_element.imgW,
...
# 找到可点击按钮的坐标数据，排除忽略按钮
def match_img2(click_img_list, ignore_img_list, ignore_img_pos_list):
    print('截图...')
    command.do_screen_cap(local_save_dir, SCREEN_CAP_IMAGE_NAME)
    # 用cv2库解析截屏图片
    screen_cap_img = cv2.imread(SCREEN_CAP_IMAGE_NAME, -1)
